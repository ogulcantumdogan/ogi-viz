...
export default class ReglVisualizer {
  constructor(audioContext, reglInstance, opts = {}) {
    this.opts = opts;
    ...
    // Default base values for presets
    ...
    // Initialize the Regl renderer
    this.renderer = new ReglRenderer(this.regl, this.audio, opts);
  }
  ...
  createPerFramePool(baseVals) {
    ...
    const globalPerFrameVars = [
      "old_wave_mode", "frame", "time", "fps", "bass", "bass_att",
      "mid", "mid_att", "treb", "treb_att", "meshx", "meshy",
      "aspectx", "aspecty", "pixelsx", "pixelsy", "rand_start", "rand_preset"
    ];
  ...
  createPerPixelPool(baseVals) {
    ...
    const globalPerPixelVars = [
      ...
      "pixelsx", "pixelsy", "rand_start", "rand_preset",
      "x", "y", "rad", "ang"
    ];
  ...
  async loadPreset(presetMap, blendTime = 0) {
    ...
    preset.baseVals = ReglVisualizer.overrideDefaultVars(
      this.baseValsDefaults,
      preset.baseVals
    );
    
    for (let i = 0; i < preset.shapes.length; i++) {
      preset.shapes[i].baseVals = ReglVisualizer.overrideDefaultVars(
        this.shapeBaseValsDefaults,
    ...
    for (let i = 0; i < preset.waves.length; i++) {
      preset.waves[i].baseVals = ReglVisualizer.overrideDefaultVars(
        this.waveBaseValsDefaults,
  ...
  async loadWASMPreset(preset, blendTime) {
    ...
    // Setup preset with WASM functions
    const handleEmptyFunction = (f) => f ? f : () => {};
  ...
  createCustomShapePerFramePool(baseVals) {
    ...
    const globalShapeVars = [
      "frame", "time", "fps", "bass", "bass_att", "mid", "mid_att",
      "treb", "treb_att", "meshx", "meshy", "aspectx", "aspecty",
      "pixelsx", "pixelsy", "rand_start", "rand_preset", "instance"
    ];
